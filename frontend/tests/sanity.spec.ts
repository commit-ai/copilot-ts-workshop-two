...
test('activity log explains when a path filter has no matching requests', async ({ page }) => {
  await page.goto('/');

  await page.getByLabel('Filter by path').fill('/api/does-not-exist');
  await expect(page.getByText('No activity matches this filter.', { exact: true })).toBeVisible();
});
